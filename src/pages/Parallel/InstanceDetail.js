...
import { Button, Card, Row, Col, Divider, Progress, Table, Badge } from 'antd';
  ...
  <Row>
    ...
    <Col xs={24} sm={12}>
      ...
      <div className={styles.heading}>
        {`${moment(time.end).diff(moment(time.start), 'minutes')}min`}
      </div>
    </Col>
  </Row>
);
...
class InstanceDetail extends PureComponent {
  componentDidMount() {
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <Card title="Task">
          <Table pagination={false} dataSource={advancedOperation1} columns={columns} />
        </Card>
      </PageHeaderWrapper>
    );
  }
