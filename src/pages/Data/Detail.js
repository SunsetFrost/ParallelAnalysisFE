...
const headerContent = (detail) => (
  <div>
    <DescriptionList className={styles.headerList} size="small" col="2">
      ...
      <Description term="Latitude Range">{`${detail.meta.spatial.lat.start}°, ${detail.meta.spatial.lat.end}°`}</Description>
      <Description term="Latitude Resolution">{`${detail.meta.spatial.lat.resolution}°`}</Description>
      <Description term="Longitude Range">{`${detail.meta.spatial.lon.start}°, ${detail.meta.spatial.lon.end}°`}</Description>
      <Description term="Longtitude Resolution">{`${detail.meta.spatial.lon.resolution}°`}</Description>
    ...
    <DescriptionList className={styles.headerList} size="small" col="1">
      <Description term="Description" style={{marginTop: '12px'}}>{detail.desc}</Description>
    </DescriptionList>
  </div>
)
...
export default class DataDetail extends PureComponent {
  ...
  render() {
    const { data: { detail, list }} = this.props;
        ...
        render: (path) => (
          <Fragment>
            <Tooltip title="download">
              <Button icon="cloud-download" onClick={() => {this.onDownloadFile(path)}}></Button>
            </Tooltip>
